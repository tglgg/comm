            ...
            if (sp1.IsOpen)     //此处可能没有必要判断是否打开串口，但为了严谨性，我还是加上了
            {
                ...
                else                                            //'发送16进制按钮'
                {
                    try
                    {
                        ...
                        txtReceive.Text += strRcv + "\r\n";
                        if (receivedData[0] == 0xeb && receivedData[1] == 0x81)
                        {
                            strRcv = receivedData[2].ToString("X2") + receivedData[3].ToString("X2");
                            uint num = uint.Parse(strRcv, System.Globalization.NumberStyles.AllowHexSpecifier);

                            byte[] floatVals = BitConverter.GetBytes(num);
                            int t = BitConverter.ToInt32(floatVals, 0);
                            float f = Convert.ToInt16(t);
                   
                            txTemp1.Text = f.ToString();
                        }
                        else if (receivedData[0] == 0xeb && receivedData[1] == 0x82)
                        {
                            for (int i = 2; i < receivedData.Length; i++) //窗体显示
                            {
